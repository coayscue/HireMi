...
//  HireMi
//
//  Created by Christian Ayscue on 5/23/15.
...
class NewListingViewController: UITableViewController, UITextViewDelegate, UITextFieldDelegate, MKMapViewDelegate, CLLocationManagerDelegate{
...
//    }
    ...
    @IBAction func createListing(sender: AnyObject) {
        ...
        var geoLoc = PFGeoPoint(latitude: 37.348990, longitude: -121.939181)
        //PFGeoPoint(latitude: map.centerCoordinate.latitude, longitude: map.centerCoordinate.longitude)
        //check to make sure all fields are filled, and the location is good
        println(jobName+pay+description+fullName+cellNum+email)
        if (jobName == "" || pay == "$0.00" || fullName == "" || cellNum == ""){
...
//            var alert = UIAlertController(title: "Location not loaded", message: "", preferredStyle: UIAlertControllerStyle.Alert)
//            var okAction = UIAlertAction(title: "Ok", style: UIAlertActionStyle.Cancel, handler: { (action) -> Void in
//                alert.dismissViewControllerAnimated(true, completion: nil)
//            })
//            alert.addAction(okAction)
...
//            
//            var wheelView = UIImageView(image: UIImage(named: "saving_box.png"))
...
//                wheelView.frame = CGRectMake(self.view.frame.size.width/3, self.view.frame.size.height/2-self.view.frame.size.width/6, self.view.frame.size.width/3, self.view.frame.size.width/3)
//                activityIndicator.frame = CGRectMake(self.view.frame.size.width/3+30, self.view.frame.size.height/2-self.view.frame.size.width/6+30, self.view.frame.size.width/3-60, self.view.frame.size.width/3-60)
//                wheelView.alpha = 1
//            })
            var savingAlert = UIAlertController(title: "Saving...", message: nil, preferredStyle: UIAlertControllerStyle.Alert);
...
//                    job["Email"] = email
                    ...
                    var newListing: [String:AnyObject] = ["Job_Name":jobName,"Pay_Type": self.payType,"Pay":pay,"Description":description,"Full_Name":fullName,"Cell_Num":cellNum,"Latitude":geoLoc.latitude, "Longitude":geoLoc.longitude, "ParseID":job.objectId]
                    ...
                    savingAlert.dismissViewControllerAnimated(false, completion: { () -> Void in
                        //present the alert
                        var alert = UIAlertController(title: "Job Requested", message: "A worker will text you shortly!", preferredStyle: UIAlertControllerStyle.Alert)
                        var okAction = UIAlertAction(title: "Ok", style: UIAlertActionStyle.Cancel, handler: { (action) -> Void in
                            alert.dismissViewControllerAnimated(true, completion: nil)
                        })
                        alert.addAction(okAction)
                        ...
                        self.payLabel.text = "Pay: $0.00"
                        self.descriptionField.text = ""
                    })
//                    activityIndicator.stopAnimating()
//                    wheelView.image = UIImage(named: "job_posted_box.png")
//                    UIView.animateWithDuration(0.2, delay: 1, options: UIViewAnimationOptions.AllowAnimatedContent, animations: { () -> Void in
//                        wheelView.frame = CGRectMake(self.view.frame.size.width/2, self.view.frame.size.height/2, 0, 0)
//                        wheelView.alpha = 0
//                        //reset the table
                    
//                    }, completion: { (blah) -> Void in
...
//                    })
                    ...
                    savingAlert.dismissViewControllerAnimated(false, completion: { () -> Void in
                        //present the alert
                        var alert = UIAlertController(title: "Error", message: "An error occurred while posting your job. Please try again.", preferredStyle: UIAlertControllerStyle.Alert)
                        var okAction = UIAlertAction(title: "Ok", style: UIAlertActionStyle.Cancel, handler: { (action) -> Void in
                            alert.dismissViewControllerAnimated(true, completion: nil)
                        })
                        alert.addAction(okAction)
